    ...
    private void Refresh() {
        ...
        if (!_mesh.isReadable) {
            if (gameObject.isStatic) {
                ...
                const string m =
                    "<b>[Quibli]</b>Curve Renderer mesh is not readable and object is static. Disabling static flag.";
                Debug.LogWarning(m);
            } else {
                const string m =
                    "<b>[Quibli]</b>Curve Renderer mesh is not readable and object is not static. Cannot update mesh.";
                Debug.LogWarning(m);
                return;
            }
        }
